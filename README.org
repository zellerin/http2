...
*** Server
Rudimentary https server (that signals h2 support with ALPN) is implemented; Key and cert are needed. It handles requests one by one except on sbcl; this is trivial to improve in several ways (thread for request, workers, polling...) but out of scope of this.
