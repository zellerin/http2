...
Having said that, the server (on sbcl, my machine, trivial page, single
requestor, 10k requests, over TLS) seems to be a bit less than twice the speed
of Hunchentoot based on h2load tool, more than twive when multiple requests are
allowed in parallel. Run or modify the [[file:scaffolding/speed-test.lisp][speed test code]] for your results:
